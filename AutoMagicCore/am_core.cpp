...
void AUTOMAGIC_CORE::StateChart_Sound_OnOff(double Vol_Raw)
{
	...
	********************/
	switch(State_SoundOnOff){
		...
		case STATE_SOUND_ON:
			if(	( Vol_Raw < VOLTHRESH__ON_TO_OFF_LOW ) ||														// 「反応良くするための仕掛け」で引っかからなかった場合の網. 急峻なtanが取れない可能性があるので.
				( (Vol_Raw < VOLTHRESH__ON_TO_OFF_HIGH) && (Vol_Raw - Last_Vol_Raw < TANTHRESH__ON_TO_OFF) )	// 反応良くするための仕掛け. Vol下りきってないが、十分低く、tanが急峻.
				 ){
				State_SoundOnOff = STATE_SOUND_OFF;
			}
